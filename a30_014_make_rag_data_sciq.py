...
def combine_columns(row: pd.Series, dataset_type: str = "sciq_qa") -> str:
    """複数列を結合して1つのテキストにする"""
...
def validate_sciq_data_specific(df) -> List[str]:
    ...
    if 'correct_answer' in df.columns:
        answer_lengths = df['correct_answer'].astype(str).str.len()
...
def show_usage_instructions(dataset_type: str = "sciq_qa") -> None:
    """使用方法の説明を表示"""
    st.markdown("---")
    st.subheader("📖 使用方法")
    st.markdown(f"""
    ...
    2. **CSVファイルアップロード**: question, correct_answer 列を含むCSVファイルを選択
...
def main():
    ...
    try:
        st.set_page_config(
            page_title="科学・技術QAデータ前処理（完全独立版）",
            page_icon="🔬",
            layout="wide",
    ...
    # ヘッダー
    st.title("🔬 科学・技術QAデータ前処理アプリ（SciQ）")
